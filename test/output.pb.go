...
type TestStruct struct {
	...
	StandardValue	string	`protobuf:"bytes,1,opt,name=StandardValue,proto3" json:"StandardValue,omitempty" nonsecret:"true"`
...
func (x *TestStruct) String() string {
	...
	for i := 0; i < ift.NumField(); i++ {
		v := ift.Field(i)
		el :=
			reflect.Indirect(ifv.Elem().FieldByName(v.Name))
		switch el.Kind() {
		case reflect.String:
			if el.CanSet() {
				tag := v.Tag.
